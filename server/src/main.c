...
static struct dc_application_lifecycle *create_application_lifecycle(const struct dc_posix_env *env,
                                                                     struct dc_error           *err);
...
static void read_displayer(const struct dc_posix_env *env,
                           struct dc_error           *err,
                           const uint8_t             *data,
                           size_t                     count,
                           size_t                     file_position,
...
static struct dc_application_settings *create_settings(const struct dc_posix_env *env, struct dc_error *err)
{
    ...
    static const uint16_t        default_port     = 2007;
...
#pragma GCC diagnostic ignored "-Wdeclaration-after-statement"
    struct options opts[] = {
        ...
        {(struct dc_setting *)settings->hostname,
         ...
         dc_string_from_config,
         default_hostname},
        {(struct dc_setting *)settings->ip_version,
         dc_options_set_regex,
         "ip",
         required_argument,
         'i',
         "IP",
        ...
        {(struct dc_setting *)settings->port,
         ...
         &default_port},
        {(struct dc_setting *)settings->reuse_address,
         dc_options_set_bool,
         "force",
         no_argument,
         'f',
...
static void do_bind(const struct dc_posix_env *env, struct dc_error *err, void *arg)
{
    ...
    dc_network_bind(env, err, app_settings->server_socket_fd, app_settings->result->ai_addr, port);
}
...
__attribute__((unused)) static void write_displayer(__attribute__((unused)) const struct dc_posix_env *env,
                                                    __attribute__((unused)) struct dc_error           *err,
                                                    const uint8_t                                     *data,
                                                    size_t                                             count,
                                                    __attribute__((unused)) size_t                     file_position,
                                                    __attribute__((unused)) void                      *arg)
{
...
__attribute__((unused)) static void read_displayer(const struct dc_posix_env     *env,
                                                   struct dc_error               *err,
                                                   const uint8_t                 *data,
                                                   size_t                         count,
                                                   __attribute__((unused)) size_t file_position,
